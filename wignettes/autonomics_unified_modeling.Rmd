...
object <- read_metabolon(file)
object %<>% lm.(   ~ subgroup)
object %<>% limma(~ subgroup) # prefers large effects over small sds
...
```{r}
object <- read_metabolon(file)
object %<>% limma(~0+subgroup, contrasts = c('t1-t0', 't2-t0', 't3-t0'), block = 'Subject')
object %<>% limma(~0+subgroup, contrasts = c('t1-t0', 't2-t1', 't3-t2'), block = 'Subject')
```
...
```{r}
object <- read_metabolon(file)
object %<>% limma(~subgroup, block = 'Subject')  # baseline diffs
object %<>% limma(~subgroup, block = 'Subject', codingfun = code_diff)  # backward diffs
```
...
```{r}
object <- read_metabolon(file)
object %<>% limma(   ~0+subgroup, contrasts = c('t3-t0'), block = 'Subject')
