...
object <- read_metabolon(file)
object %<>% linmod_lm(   ~ subgroup)
object %<>% linmod_limma(~ subgroup) # prefers large effects over small sds
...
object %<>% linmod_lme(  ~ subgroup, block = 'Subject')
object %<>% linmod_lmer( ~ subgroup, block = 'Subject') # results identical for this basic 
plot_contrast_venn(is_sig(object, fit = fits(fdt(object)), contrast = 't3'))
# plot_contrast_boxplots(object, subgroupvar='SET', contrast='t3', fit = c('lm', 'limma'))
```
...
```{r}
object <- read_metabolon(file)
object %<>% linmod_limma(~0+subgroup, contrasts = c('t1-t0', 't2-t0', 't3-t0'), block = 'Subject')
object %<>% linmod_limma(~0+subgroup, contrasts = c('t1-t0', 't2-t1', 't3-t2'), block = 'Subject')
```
...
```{r}
object <- read_metabolon(file)
object %<>% linmod_limma(~subgroup, block = 'Subject')  # baseline diffs
object %<>% linmod_limma(~subgroup, block = 'Subject', codingfun = code_diff)  # backward diffs
```
...
```{r}
object <- read_metabolon(file)
object %<>% linmod_limma(   ~0+subgroup, contrasts = c('t3-t0'), block = 'Subject')
object %<>% linmod_wilcoxon(~0+subgroup, contrasts = c('t3-t0'), block = 'Subject')
