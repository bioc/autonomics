...
p2 <- plot_summarized_detections(object)
...
`plot_summarized_detections` summarizes NA structure at the **subgroup** level, differentiating systematic NA values (white) from random NA values and full detections (color).
...
```{r, echo = FALSE, message = FALSE}
  ...
  systime <- system.time(object %<>% limo_lm())
```
...
```{r, eval = FALSE}
object %<>% limo_lm()
object %<>% limo_lm(formula = ~ subgroup)
```
...
file <- system.file('extdata/fukuda20.proteingroups.txt', package = 'autonomics')
object <- read_maxquant_proteingroups(file)
object %<>% limo_limma(formula   = ~ 0 + subgroup, contrasts = c('Adult - X30dpt'))
```
# References
