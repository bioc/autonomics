...
# Difference Contrasts
    ...
    object %<>% limma(formula = formula, block = block,  coefs = coefs)
# Treatment Contrasts
    coefs <- character(0)
    for (groupvar in groupvars){
        levels <- base::levels(object[[groupvar]])
...
mobj %<>% limma(     formula = ~ Time + Diabetes, block = 'Subject', coefs = c('t1-t0', 't2-t1', 't3-t2', 'T2DM'))
pobj %<>% limma(     formula = ~ Time + Diabetes, block = 'Subject', coefs = c('t1-t0', 't2-t1', 't3-t2', 'T2DM'))
...
# D : t1 - t0
mobj %<>% limma(formula = ~ Diabetes/Time, block = 'Subject')
pobj %<>% limma(formula = ~ Diabetes/Time, block = 'Subject')
...
contrasts(mobj$subgroup) <- MASS::contr.sdif(levels(mobj$subgroup))
mobj %<>% limma(formula = ~ subgroup/Diabetes, block = 'Subject')
...
mobj |> limma(formula = ~ Diabetes/subgroup, block = 'Subject') |>
