...
# Difference Contrasts
    ...
    object %<>% limo_limma(formula = formula, block = block,  coefs = coefs)
# Treatment Contrasts
    coefs <- character(0)
    for (groupvar in groupvars){
        levels <- base::levels(object[[groupvar]])
...
mobj %<>% limo_limma(     formula = ~ Time + Diabetes, block = 'Subject', coefs = c('t1-t0', 't2-t1', 't3-t2', 'T2DM'))
pobj %<>% limo_limma(     formula = ~ Time + Diabetes, block = 'Subject', coefs = c('t1-t0', 't2-t1', 't3-t2', 'T2DM'))
...
# D : t1 - t0
mobj %<>% limo_limma(formula = ~ Diabetes/Time, block = 'Subject')
pobj %<>% limo_limma(formula = ~ Diabetes/Time, block = 'Subject')
...
contrasts(mobj$subgroup) <- MASS::contr.sdif(levels(mobj$subgroup))
mobj %<>% limo_limma(formula = ~ subgroup/Diabetes, block = 'Subject')
...
mobj |> limo_limma(formula = ~ Diabetes/subgroup, block = 'Subject') |>
