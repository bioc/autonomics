...
#============================================================================
    ...
    test_that( " fit: billing19.rnacounts ", {
        ...
        expect_true(sumexp_contains_fit(limo_limma(object),                   'limma'))
        expect_true(sumexp_contains_fit(limo_limma(object, weightvar = NULL), 'limma'))
        # expect_true(sumexp_contains_fit(limo_lm(object),       'lm'))         # slow
        # expect_true(sumexp_contains_fit(limo_wilcoxon(object), 'wilcoxon'))   # slow
    })
    ...
    test_that(  " fit: billing19.proteingroups ", {
        # original
            ...
            expect_true(sumexp_contains_fit(limo_wilcoxon(object), 'wilcoxon'))
            expect_true(sumexp_contains_fit(limo_lm(object),       'lm'))
            expect_true(sumexp_contains_fit(limo_limma(object),    'limma'))
        # subtracted
            obj <- object
            obj %<>% subtract_baseline('subgroup', 'E00_STD')
            expect_true(sumexp_contains_fit(limo_limma(object, ~1), 'limma'))
    })
    ...
    test_that( " fit: atkin.somascan ", {
        # Original
            ...
            expect_true( sumexp_contains_fit(    limo_limma( object, ~ Time + Diabetes, block = 'Subject' ), 'limma'   ) )
            expect_true( sumexp_contains_fit(      limo_lm( object, ~ Time + Diabetes                    ), 'lm'      ) )
            expect_true( sumexp_contains_fit(      limo_lme( object, ~ Time + Diabetes, block = 'Subject' ), 'lme'     ) )
            expect_true( sumexp_contains_fit(     limo_lmer( object, ~ Time + Diabetes, block = 'Subject' ), 'lmer'    ) )
            expect_true( sumexp_contains_fit( limo_wilcoxon( object, ~ Time,            block = 'Subject' ), 'wilcoxon') )
        # Subtracted
            object %<>% subtract_differences(block = 'Subject', subgroupvar ='Time')
            expect_true( sumexp_contains_fit(   limo_lm(object, ~ 0 + Time), 'lm'   ))
            expect_true( sumexp_contains_fit( limo_limma(object, ~ 0 + Time), 'limma'))
    })
    ...
    test_that( " fit: atkin.metabolon ", {
        ...
        # test
            expect_true(sumexp_contains_fit(                  limo_limma( object, ~ subgroup, block = 'Subject'),  'limma'   ))
            expect_true(sumexp_contains_fit(                    limo_lm( object, ~ subgroup, block = 'Subject'),  'lm'      ))
            expect_true(sumexp_contains_fit(                    limo_lme( object, ~ subgroup, block = 'Subject'),  'lme'     ))
            expect_true(sumexp_contains_fit( suppressWarnings( limo_lmer( object, ~ subgroup, block = 'Subject')), 'lmer'    ))
            expect_true(sumexp_contains_fit(               limo_wilcoxon( object, ~ subgroup, block = 'Subject'),  'wilcoxon'))
    })
